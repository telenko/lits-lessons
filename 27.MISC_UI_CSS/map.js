...
const mapSvg = document.querySelector('svg#ukraine-map');

mapSvg.addEventListener('click', (event) => {
    const triggerEle = event.target;
    if (triggerEle.tagName !== 'path') {
        return;
    }
    const regionName = triggerEle.getAttribute("name");
   
    alert(JSON.stringify(dictionary[regionName]));
});

let scale = 1;

const zoomInBtn = document.querySelector('#navigation-zoom-in')
const zoomOutBtn = document.querySelector('#navigation-zoom-out')
